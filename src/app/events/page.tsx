...
// Helper: parse string tanggal Indonesia ke timestamp untuk keperluan sorting.
// Mendukung format: "12 Agustus 2026", "5-6 Agustus 2026", atau "Agustus 2026" (tanpa tanggal -> dianggap tanggal 1).
// Jika tanggal kosong atau format tidak dikenali, dikembalikan Infinity supaya
// item tersebut otomatis terdorong ke akhir urutan (tidak mengacaukan sort).
function parseIndoDate(dateStr: string | null | undefined): number {
  ...
  if (!match) return Infinity; // format tidak dikenali

  const [, dayStr, bulanStr, yearStr] = match;
...
export default async function EventsPage() {
  ...
  // Filter events based on is_past flag, lalu urutkan berdasarkan tanggal terdekat
  const upcomingEvents = allEvents
    .filter(evt => !evt.is_past)
    .sort((a, b) => parseIndoDate(a.date) - parseIndoDate(b.date)); // tanggal terdekat dulu

  const pastEvents = allEvents
    .filter(evt => evt.is_past)
    .sort((a, b) => parseIndoDate(b.date) - parseIndoDate(a.date)); // paling baru dulu
  ...
  return (
    <div className="pt-32 pb-24 min-h-screen bg-slate-50">
      <div className="max-w-7xl mx-auto px-6">
        ...
        {/* Error state (opsional, hanya tampil jika query Supabase gagal) */}
        {hasError && (
          <div className="mb-12 bg-red-50 border border-red-200 rounded-2xl p-6 text-center">
            <p className="text-red-600 font-medium text-sm">
              Terjadi kendala saat memuat data event. Silakan muat ulang halaman ini beberapa saat lagi.
            </p>
          </div>
        )}
        ...
        <div className="mb-24">
          ...
          {upcomingEvents.length === 0 ? (
            <div className="bg-white rounded-4xl border border-slate-200 shadow-sm p-12 text-center">
              <Calendar className="w-10 h-10 text-slate-300 mx-auto mb-4" />
              <p className="text-slate-500 font-medium">
                Belum ada kegiatan mendatang yang dijadwalkan saat ini.
              </p>
            </div>
          ) : (
                ...
                >
                  <Link href={`/events/${evt.id}`} className="contents">
                    <div className="h-60 relative overflow-hidden bg-slate-100 p-3">
                      ...
                      {evt.status === 'upcoming' && (
                        <div className="absolute top-6 left-6 bg-white/95 backdrop-blur-md px-4 py-2 rounded-full text-xs font-bold text-slate-500 shadow-sm flex items-center gap-2 z-10">
                    ...
                    <div className="p-8 flex flex-col flex-1">
                      <div className="flex items-center gap-3 text-foni-orange font-bold text-xs uppercase tracking-widest mb-4">
                        ...
                        {evt.date || 'Tanggal belum ditentukan'}
                      </div>
                      <h3 className="text-2xl font-bold text-slate-900 mb-6 leading-tight group-hover:text-foni-navy transition-colors line-clamp-2">
                        {evt.title || 'Judul event belum tersedia'}
                      </h3>
                      <div className="space-y-4 mb-8 mt-auto">
                        <div className="flex items-center gap-3 text-slate-600 text-sm">
                          ...
                          <span className="font-medium line-clamp-1">{evt.location || 'Lokasi belum ditentukan'}</span>
                        </div>
                        <div className="flex items-center gap-3 text-slate-600 text-sm">
                          <div className="w-10 h-10 rounded-2xl bg-slate-50 border border-slate-100 flex items-center justify-center shrink-0">
                        ...
                        <div className="flex items-center gap-3 text-slate-600 text-sm">
                          ...
                          <span className="font-medium line-clamp-1">{evt.organizer || '-'}</span>
                        </div>
                      </div>
                    </div>
                  </Link>

                  {/* Tombol aksi, di luar Link supaya tidak nested */}
                  <div className="px-8 pb-8 -mt-2">
                    {evt.status === 'registration_open' && evt.registration_link ? (
                      <a
                        ...
                        className="cursor-pointer w-full py-4 rounded-xl font-bold text-sm transition-colors flex items-center justify-center gap-2 bg-foni-navy text-white hover:bg-slate-800"
                      >
                        Daftar Sekarang
                      ...
                      <Link
                        href={`/events/${evt.id}`}
                        className="cursor-pointer w-full py-4 rounded-xl font-bold text-sm transition-colors flex items-center justify-center gap-2 bg-slate-100 text-slate-700 hover:bg-slate-200"
                      >
                        Detail Info
                        <ArrowRight className="w-4 h-4" />
                      </Link>
                  ...
                  >
                    <div>
                      <div className="flex items-center gap-3 mb-2 text-xs font-bold uppercase tracking-widest">
                        ...
                        <div className="flex items-center gap-1 text-slate-400">
                          ...
                          {evt.date || 'Tanggal tidak tercatat'}
                        </div>
                      </div>
                    ...
                    <div className="flex flex-col sm:items-end gap-2 shrink-0">
                      <div className="inline-flex items-center gap-2 bg-white px-3 py-1.5 rounded-lg border border-slate-200 text-xs font-bold text-slate-600">
                        <MapPin className="w-3 h-3 text-foni-orange" /> {evt.location || 'Lokasi belum ditentukan'}
                      </div>
