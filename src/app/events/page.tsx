        ...
        <div className="mb-24">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {upcomingEvents.map((evt) => (
              <div
                key={evt.id}
                className="bg-white rounded-[2rem] overflow-hidden border border-slate-200 shadow-sm hover:shadow-xl hover:shadow-slate-200/50 hover:border-[var(--color-foni-navy)] group hover:-translate-y-2 transition-all duration-500 flex flex-col"
              >
                <div className="h-60 relative overflow-hidden bg-slate-100 p-3">
                  ...
                  {evt.status === 'registration_open' && (
                    <div className="absolute top-6 left-6 bg-white/95 backdrop-blur-md px-4 py-2 rounded-full text-xs font-bold text-[var(--color-foni-navy)] shadow-sm flex items-center gap-2 z-10">
                ...
                <div className="p-8 flex flex-col flex-1">
                  ...
                  <button className={`w-full py-4 rounded-xl font-bold text-sm transition-colors flex items-center justify-center gap-2 ${
                    evt.status === 'registration_open' 
                      ? 'bg-[var(--color-foni-navy)] text-white hover:bg-slate-800' 
          ...
          <div className="bg-white rounded-[2rem] border border-slate-200 shadow-sm p-6 md:p-10">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {pastEvents.map((evt) => (
                <div 
                  key={evt.id}
                  className="group flex flex-col sm:flex-row sm:items-center justify-between gap-6 p-6 rounded-2xl border border-slate-100 bg-slate-50 hover:bg-white hover:border-[var(--color-foni-navy)] hover:shadow-md transition-all duration-300"
                >
                  <div>
                    ...
                    <h3 className="text-xl font-bold text-slate-800 group-hover:text-[var(--color-foni-navy)] transition-colors mb-4 sm:mb-0">
                      {evt.title}
                    </h3>
                  </div>
                  ...
                  <div className="flex flex-col sm:items-end gap-2 shrink-0">
                    <div className="inline-flex items-center gap-2 bg-white px-3 py-1.5 rounded-lg border border-slate-200 text-xs font-bold text-slate-600">
                      <MapPin className="w-3 h-3 text-[var(--color-foni-orange)]" /> {evt.location}
                    </div>
                    <div className="inline-flex items-center gap-2 bg-[var(--color-foni-navy)]/5 px-3 py-1.5 rounded-lg border border-[var(--color-foni-navy)]/10 text-xs font-bold text-[var(--color-foni-navy)]">
